...
function modifySearch() {
    ...
    btn.addEventListener('click', (e) => {
        ...
        fetch(`https://18.223.117.204:80/products/${id}`)
            .then((resp) => resp.json())
            .then(data => {
                const product = data
...
function deleteSend() {
    ...
    btn.addEventListener('click', (e) => {
        ...
        fetch(`https://18.223.117.204:80/products/${id}`, { method: 'DELETE' })
...
function renderSales() {
    ...
    btn.addEventListener('click', () => {
        ...
        fetch(`https://18.223.117.204:80/sales`)
            .then((resp) => resp.json())
            .then(data => {
                const containerCard = document.getElementById('containerCard')
                while (containerCard.firstChild) {
...
const detectBtnDetail = () => {
    ...
    btn.forEach(btn => {
        btn.addEventListener('click', () => {
            let producto = {}
            fetch(`https://18.223.117.204:80/products/${btn.dataset.id}`)
                .then((resp) => resp.json())
                .then(data => {
                    const product = data
